...
def process_tile(row):
    ...
    if config["CREATE_TILE_MAG1C"]:
        ...
        arg_list = []
        arg = [sys.executable, "benchmark/mag1c_fork/mag1c/mag1c.py", f"{name}","-o", "--use-wavelength-range", str(config["wavelengths_range"][0]), str(config["wavelengths_range"][1])]
        arg_list.append(arg)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run HSI processing script")
    parser.add_argument(
        "--config",
        type=str,
        default="benchmark/cfg/classic.yaml",
        help="Path to YAML config file"
    )
    args = parser.parse_args()
    with open(args.config, 'r') as f:
        config_base = yaml.safe_load(f)
    # Convert list to tuple where needed
    if "wavelengths_range" in config_base:
        config_base["wavelengths_range"] = tuple(config_base["wavelengths_range"])
